...
export const RGBInterface = (props: InterfaceProps) => {
  ...
  const cssbg = `rgba(${rgba[0] * 255}, ${rgba[1] * 255}, ${rgba[2] * 255}, ${opacity})`
  ...
  const onHandleSatLightChange = (coord: number[], bounds: number[]) => {
    ...
    const newLight = Math.max(0, Math.min(1, coord[0] / bounds[0] * (1 - newSat / 2)))
    ...
    <Flex flexDir="column" width="100%" height="100%" maxH="100%">
      ...
      <SVGBox
        ...
        withSvg={(width, height) => (
          <>
            <defs>
              <clipPath id="clip">
                <rect x={1} y={1} width={width - 1} height={height - 1} r={half} />
              </clipPath>
            ...
            <g id="sat-light-spectrum" opacity={opacity} clipPath="url(#clip)">
              ...
              <rect x={0} y={0} width={width} height={height} fill="url('#g2')" style={{ mixBlendMode: "multiply" }} />
            </g>
            <circle cx={satLightCursor[0] * width} cy={satLightCursor[1] * height} r={10} fill={cssbg} stroke={bg} strokeWidth="2px" />
          </>
        )} />
      <Box px="8px">
        <SVGBox
          ...
          withSvg={(width, height) => (
            <>
              ...
              <line x1={hueCursor * width} x2={hueCursor * width} y1={-1} y2={height + 1} stroke={bg} strokeWidth="6px" />
            </>
          )}>
      ...
      <Box width="100%" height="40%" p="8px" minH="40px">
        <Flex h="100%" w="100%" fontSize="10px">
          ...
          <Flex bg={themed('bg')} w="100%" h="100%" flexDir="column" whiteSpace="nowrap" pt="4px">
            <Label text="HSL" w="100%" px="0.5rem" py="2x">
              <Text wrap="nowrap" fontSize="10px" color={themed("textMidLight")}>
                {Math.round(hsl[0] * 360)}° {hsl[1].toFixed(2)} {hsl[2].toFixed(2)}
              </Text>
            </Label>
