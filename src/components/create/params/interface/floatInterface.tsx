...
export const InterfaceKnob = (props: { isInt?: boolean } & InterfaceProps) => {
  ...
  const [{ scale = 1, isNeg = true }, setPropValue, _c] =
    useInterfaceProps(props);
  ...
  const toParamSpace = (svgCoord: number[]) => {
    ...
    const norm = Math.min(
      1,
      Math.max(0, ((angle / (2 * Math.PI) - 0.125) * 4) / 3)
    );
  ...
  const paramAngle =
    (5 * Math.PI) / 4 - (((clampedVal - min) / (max - min)) * 3 * Math.PI) / 2;
  ...
  const knobRingOuter = useMemo(
    () => `
  M ${-ringOuterDiag} ${ringOuterDiag} A ${ringOuter} ${ringOuter} 0 ${
      paramAngle > Math.PI / 4 ? 0 : 1
    } 1 ${ringOuter * paramFactorX} ${-ringOuter * paramFactorY}
  L ${ringEdgeInner * paramFactorX} ${-ringEdgeInner * paramFactorY}
  M ${ringEdgeInner * paramFactorX} ${
      -ringEdgeInner * paramFactorY
    } A ${ringEdgeInner} ${ringEdgeInner} 1 ${
      paramAngle > Math.PI / 4 ? 0 : 1
    } 0 ${-ringEdgeInnerDiag} ${ringEdgeInnerDiag}
  L ${-ringOuterDiag} ${ringOuterDiag}
  `,
    [
      ringEdgeInner,
  ...
  return (
    <>
      <svg
        width={size}
        height={size}
        viewBox={`-${half + 8} -${half + 8} ${size + 16} ${size + 16}`}
        ref={ref}
      >
        <defs>
        ...
        <text
          ...
          pointerEvents="none"
        >
          <tspan x={0} y={4} textAnchor="middle">
            {fmt.format(paramVal)}
          </tspan>
        </text>
        <text fill={s1} fontSize={8} fontFamily={fontMono} pointerEvents="none">
          <tspan
            x={(ringOuterDiag + ringInnerDiag) / 2 + 12}
            y={(ringOuterDiag + ringInnerDiag) / 2}
            textAnchor="middle"
          >
            {fmt.format(max)}
          </tspan>
        ...
        {Array(ticks + 1)
          ...
          .map((_, idx) => {
            ...
            const lineEnd = large
              ? (ringOuter + ringInner) / 2
              : ringInner + (ringOuter - ringInner) / 4;
      ...
      >
        <Button
          variant="empty"
          size="xs"
          onClick={() => setScale(Math.max(scale - 1, props.isInt ? 0 : -5))}
          p="0"
          fontWeight="bold"
        >
          -
        ...
        <Button
          variant="empty"
          size="xs"
          onClick={() => setScale(Math.min(12, Math.max(scale + 1, -5)))}
          p="0"
          fontWeight="bold"
        >
          +
...
export const IntInterfaceKnob = (props: InterfaceProps) => (
  <InterfaceKnob {...props} isInt />
);
