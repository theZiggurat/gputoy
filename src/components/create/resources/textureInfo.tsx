      ...
      <Label text="Dimension" {...labelProps}>
        <chakra.select
          ...
          onChange={(s) =>
            props.onChange({
              ...props.args,
              dim: s.target.value as types.TextureDim,
            })
          }
          sx={bgProps}
          paddingStart="0.5rem"
        ...
        >
          ...
          <option value="2d" style={optionStyle}>
            2D
          </option>
          <option value="3d" style={optionStyle}>
            3D
          </option>
        </chakra.select>
      </Label>
      ...
      <Label text="Width" {...labelProps}>
        <InputGroup size="xs">
          <Input
            ...
            {...inputProps}
          />
          <InputRightElement color={themed("textLight")}>px</InputRightElement>
        </InputGroup>
      </Label>
          ...
          <InputGroup size="xs">
            <Input
              ...
              onChange={(s) =>
                props.onChange({
                  ...props.args,
                  height: parseInt(s.target.value),
                })
              }
              max={8192}
              {...inputProps}
      ...
      <Label text="Format" {...labelProps}>
        <chakra.select
          ...
          onChange={(s) =>
            props.onChange({
              ...props.args,
              formatType: s.target.value as types.TextureFormatType,
            })
          }
          sx={bgProps}
          paddingStart="0.5rem"
        ...
        >
          {types.TEXTURE_FORMAT_TYPES.map((t) => (
            <option key={t} title={t} value={t} style={optionStyle}>
              {t}
            </option>
          ))}
        </chakra.select>
        {types.TEXTURE_FORMAT_FAMILIES[formatType] && (
          <chakra.select
            ...
            onChange={(s) =>
              props.onChange({
                ...props.args,
                format: s.target.value as GPUTextureFormat,
              })
            }
            sx={bgProps}
            paddingStart="0.5rem"
        ...
        <Label text="Samples" {...labelProps}>
          <Input
            ...
            onChange={(s) =>
              props.onChange({
                ...props.args,
                sampleCount: parseInt(s.target.value),
              })
            }
            max={16}
            {...inputProps}
