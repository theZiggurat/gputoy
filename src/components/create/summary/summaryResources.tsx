...
import {
    ...
    resourceKeysAtom,
    withAddResource
} from "@core/recoil/atoms/resource";
...
const ResourceConstructor = (props: {
  ...
  return (
    <form onSubmit={onHandleUserSubmit}>
      <Flex pl="1rem" flexDir="row" alignItems="center">
        <Icon
          as={typeToIcon[type]}
          size={10}
  ...
  return (
    ...
    >
      <Flex gridGap="0.5rem" alignItems="center">
        <Icon as={typeToIcon[type]} size={10} transform="translate(1px, 0)" />
        <Text fontSize="13px" userSelect="none" textOverflow="ellipsis">
          {name}
        </Text>
      </Flex>
      <IconButton
...
const SummaryResources = (props: { instanceId: string }) => {
  ...
  return (
    <Flex flexDir="column" fontSize="xs" color={themed("textMid")}>
      <Box h="0.5rem" />
      {resourceKeys.map((k) => {
        return (
          <ResourceEntry
