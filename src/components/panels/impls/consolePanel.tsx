...
const ConsolePanel = (props: DynamicPanelProps & any) => {
  const [instanceState, setInstanceState] =
    useInstance<ConsoleInstanceState>(props);
  const console = useConsole(
    instanceState.typeFilters,
    instanceState.keywordFilter
  );
  const clear = useClearConsole();

  const setKeywordFilter = (filter: string) =>
    setInstanceState({ ...instanceState, keywordFilter: filter });
  const setTypeFilters = (filter: boolean[]) =>
    setInstanceState({ ...instanceState, typeFilters: filter });
  ...
  const writeToClipboard = () => {
    navigator.clipboard.writeText(
      console
        .map(
          (line) =>
            `${formatTime(line.time)}  ${prehead[line.type]} ${line.header}: ${
              line.body
            }`
        )
        .join("\n")
  ...
  return (
    <Panel {...props}>
      <PanelContent fontFamily={fontMono} fontSize="sm" p="0.3rem">
        {console.map((message: Message, idx) => (
          ...
          >
            <Box ml={1}>
              ...
              <Text
                color={colors[Math.log2(message.type)].concat(
                  consoleFontColorMod
                )}
                ...
                display="inline"
              >
                {prehead[Math.log2(message.type)]}
                {message.header}:&nbsp;
              </Text>
              <Text display="inline">{message.body}</Text>
            </Box>
          </Box>
        ))}
        <div ref={bottom}></div>
      ...
      <PanelBar>
        ...
        <PanelBarMiddle>
          <InputGroup maxWidth="500" minWidth="100" size="xs">
            ...
            {instanceState.keywordFilter.length > 0 && (
              <InputRightElement onClick={() => setKeywordFilter("")}>
                <CloseIcon size="sm" />
              </InputRightElement>
            )}
          </InputGroup>
          ...
          <RowToggleButton
            text="Trace"
            toggled={instanceState.typeFilters[0]}
            onClick={() => toggle(0)}
          />
          <RowToggleButton
            text="Log"
            toggled={instanceState.typeFilters[1]}
            onClick={() => toggle(1)}
          />
          <RowToggleButton
            text="Error"
            toggled={instanceState.typeFilters[2]}
            onClick={() => toggle(2)}
          />
          <RowToggleButton
            text="Fatal"
            toggled={instanceState.typeFilters[3]}
            onClick={() => toggle(3)}
          />
          <RowToggleButton
            text="Debug"
            toggled={instanceState.typeFilters[4]}
            onClick={() => toggle(4)}
            last
          />
        </PanelBarMiddle>
        ...
        <PanelBarEnd>
          ...
          <RowButton
            ...
            disabled={console.length == 0}
          />
          <RowButton purpose="Console Settings" icon={<MdSettings />} last />
        </PanelBarEnd>
      </PanelBar>
    </Panel>
  );
