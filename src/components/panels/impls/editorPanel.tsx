...
import { conf, monarchLanguage } from "../../../monaco/wgsl";
...
const EditorContent = (props: {
  ...
  return (
    <Box width="100%" flex="1 1 auto" overflowX="hidden" overflowY="hidden">
      {isText(extension) && (
        <Editor
          height="100%"
          path={props.fileId}
...
const EditorPanel = (props: PanelInProps) => {
  const [instanceState, setInstanceState] =
    useInstance<EditorInstanceState>(props);
  const { workspace, currentFileIndex } = instanceState;
  const currentFileId =
    currentFileIndex !== undefined ? workspace[currentFileIndex] : undefined;
  ...
  const addFileToWorkspace = (localId: string, move?: boolean) => {
    ...
    if (index < 0) {
      setInstanceState((old) => {
        return {
          currentFileIndex: move ? len : old.currentFileIndex,
          workspace: old.workspace.concat(localId),
        };
      });
