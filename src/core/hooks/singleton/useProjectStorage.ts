...
const useProjectStorage = (props: ProjectStorageProps) => {
  const { projectFromDB, dateInfo, session } = props;
  ...
  useEffect(() => {
    if (isSet) clearTimeout(timeoutId.current);
  }, [isSet]);

  useEffect(() => {
    const projectFromStorage = localStorage.getItem(
      `project_local_${projectID}`
    );
    ...
    if (!projectFromStorage) {
      logger.log(
        "Serializer",
        `Using project from server: ${projectFromDB!.title}`
      );
      setProjectState(projectFromDB!);
      if (dateInfo) setProjectLastSave(dateInfo.updatedAt);
    } else {
      const { updatedAt, ...project } = JSON.parse(projectFromStorage);
      logger.log(
        "Serializer",
        `Using project from local storage: ${project.title}`
      );
  ...
  const saveToLocalStorage = () => {
    ...
    localStorage.setItem(
      `project_local_${projectID}`,
      JSON.stringify(projectWithDate)
    );
  };
