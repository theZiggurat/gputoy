import {
  projectFileDataAtom,
  projectFileMetadataAtom,
  projectFilesListAtom,
  withFileSetter,
  withProjectFilesMetadata,
} from "@core/recoil/atoms/files";
...
export const useFile = (fileId: string): FileProps => {
  const [fileMetadata, setFileMetadata] = useRecoilState(
    projectFileMetadataAtom(fileId)
  );
  ...
  const setMetadata = useCallback(
    (key: string, metadata: any) => {
      setFileMetadata((old) => ({
        ...old,
        metadata: { ...old.metadata, [key]: metadata },
      }));
    },
    [setFileMetadata]
  );
  ...
  return {
    file: { ...fileMetadata, data: fileData },
    setData,
    setFilename,
    setExtension,
    setPath,
...
export const useFileMetadata = (fileId?: string): FileMetadataProps => {
  const [fileMetadata, setFileMetadata] = useRecoilState(
    projectFileMetadataAtom(fileId ?? "root")
  );
...
export type DirectoryProps = {
  ...
  addFile: (
    path: string,
    infile: Omit<types.File, "id" | "path">
  ) => string | undefined;
...
export const useDirectory = (): DirectoryProps => {
      ...
      if (infile.extension !== "_UNCREATED") {
        for (const file of Object.values(files)) {
          if (file.filename === newfile.filename) {
            logger.err(
              "directory::addFile",
              `File already exists: ${file.filename}.${file.extension}`
            );
            return;
          }
        }
  ...
  const addDirectory = useCallback(
    (path: string): string | undefined => {
      // add file
      const id = nanoid(8);
      const newfile: types.File = {
        filename: "",
        id,
        path: path,
        data: "",
        extension: "_DIR_UNCREATED",
        metadata: { children: 0 },
      };
      for (const file of Object.values(files)) {
        if (file.filename === newfile.filename) {
          logger.err(
            ...
            `Name taken: ${file.filename}.${file.extension}`
          );
          return;
        }
      }
  ...
  const moveDirectory = (oldpath: string, newpath: string): boolean => {
    ...
    const newPathParts = newpath
      .split("/")
      .filter((s) => s.length > 0 && s !== "ERROR_IF_SEEN");

    for (const id of idsInChunk) {
      const file = files[id];
      const newPathPartsRelative = file.path
        .split("/")
        .filter((s) => s.length > 0)
        .slice(rootDepth);

      const newPathFull =
        "/" + newPathParts.concat(newPathPartsRelative).join("/");
