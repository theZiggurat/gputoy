...
export const useTaskPusher = (instanceId: string) => {
  ...
  return (task: Partial<types.Task>) =>
    setTasks({
      ...task,
      sourceId: instanceId,
      message: task["message"] ?? "noop",
    });
};
...
export const useTaskReciever = (
  ...
  useEffect(() => {
    if (task && task.message) {
      ...
      if (typeof taskFunc === "function") taskFunc(task);
    }
  }, [task]);
};

export const useTaskCoordinator = (logger?: Logger) => {
  const task = useRecoilValue(masterTaskAtom);
  const push = useSetRecoilState(withTaskPusher);
  const focusFIFO: types.InstanceSelector[] = useRecoilValue(
    instanceFocusPriorityAtom
  );
  const instances: types.InstanceSelector[] = useRecoilValue(
    panelInstanceListAtom
  );

  const locateTaskTarget = useCallback(
    (task: types.Task) => {
      ...
      if (targetId) {
        ...
        if (sel) {
          ...
          let firstSelAvail = instances.find(
            (sel) => sel.index === targetPanelIndex
          );
          if (!firstSelAvail) {
            logger?.debug(
              "TASK",
              `No avail recievers for index ${targetPanelIndex}`
            );
            return;
          }
