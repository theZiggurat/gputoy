 ...
 */
export const panelInstanceAtom = atomFamily<
  types.InstanceState,
  types.InstanceSelector
>({
  key: "instanceState",
...
export const withEditorLayout = selector<types.EditorLayout>({
  ...
  set: ({ set, reset }, state) => {
    ...
    Object.entries(state.instances ?? {}).forEach(([id, instanceState]) => {
      ...
      list.push({ id, index });
      set(panelInstanceAtom({ id, index }), (old) => ({
        ...old,
        ...instanceState,
      }));
    });
...
export const withInstanceCleaner = selector<types.InstanceSelector[]>({
  ...
  set: ({ set, reset }, instances) => {
    set(panelInstanceListAtom, (prev) => {
      const idArr = (instances as types.InstanceSelector[]).map(
        (sel) => sel.id
      );
      prev
        .filter((sel) => !idArr.includes(sel.id))
        .forEach((sel) => reset(panelInstanceAtom(sel)));
      return instances;
    });
  },
});
