...
export const systemValidationResultAtom = atomFamily<types.ValidationResult | undefined, string>({
  key: 'sys_validresult',
  default: undefined
})
...
export const systemNamespaceResultAtom = atomFamily<types.Namespace | undefined, string>({
...
export const withSystemPrebuildResult = selector<types.SystemPrebuildResult>({
  ...
  set: ({ set, reset }, result) => {
    ...
    for (const namespaceId of Object.keys(result.namespace)) {
      set(systemNamespaceResultAtom(namespaceId), result.namespace[namespaceId])
    }
