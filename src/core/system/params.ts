...
class Params {
  ...
  set = (
    ...
    if (!needRecompile && !this.frozen) {
      ...
      needRecompile = !inparams.every((newp) => {
        let match = this.params.find(
          (oldp) => newp.paramName === oldp.paramName
        );
  ...
  updateDesc = (device: GPUDevice, logger?: Logger) => {
    ...
    if (!device) {
      logger?.err(
        `Params_@${this.name}`,
        "No device to update param descriptors. Aborting."
      );
      return;
    }
    ...
    if (namedParams.length > 0) {
      namedParams.forEach((p, idx) => {
        this.byteOffsets[idx] =
          idx == 0
            ? 0
            : roundUp(
                SHADER_TYPE_META[p.paramType].align,
                this.byteOffsets[idx - 1] +
                  SHADER_TYPE_META[namedParams[idx - 1].paramType].size
              );
        align = Math.max(align, SHADER_TYPE_META[p.paramType].align);
      });
      let lastMemberType = namedParams[namedParams.length - 1].paramType;
      this.sizeByte = roundUp(
        align,
        this.byteOffsets[namedParams.length - 1] +
          SHADER_TYPE_META[lastMemberType].size
      );
    }
    ...
    logger?.debug(
      `Params`,
      `Buffer \`${this.name}\` created with size of ${this.sizeByte} bytes and config: \n`.concat(
        namedParams
          .map(
            (p: types.ParamDesc, idx: number) =>
              `${this.byteOffsets[idx]}\t${p.paramName}\t\t${
                SHADER_TYPE_META[p.paramType].size
              } bytes -- ${SHADER_TYPE_META[p.paramType].align} align`
          )
          .join("\n")
      )
    );
    ...
    logger?.debug(
      `Params_@${this.name}`,
      `Bind group created at binding ${this.binding}`
    );
  ...
  uploadToGPU = (device: GPUDevice) => {
    ...
    namedParams.forEach((p, idx) => {
      ...
      else floatView.set(p.param, this.byteOffsets[idx] / 4);
    });
  ...
  getShaderDecl = (lang: types.ExtensionShader): string => {
    ...
    if (lang == "wgsl") {
      ...
      namedParams.forEach((p) =>
        unidecl.push(`\t${p.paramName}: ${SHADER_TYPE_META[p.paramType].wgsl};`)
      );
      unidecl.push(
      ...
      unidecl.push(
        `layout(binding = ${this.binding}, set = ${this.group}) uniform ${this.name} {`
      );
      namedParams.forEach((p) =>
        unidecl.push(`\t${SHADER_TYPE_META[p.paramType].glsl} ${p.paramName};`)
      );
