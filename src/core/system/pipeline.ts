...
import TextureResource, {
    CanvasTextureResource
} from "./resource/textureResource";
...
export class QuadPipeline implements types.Pipeline {
  ...
  build = async (
    ...
    const fileId = Object.values(files).find(
      (f) => f.filename === filename
    )?.id;
    if (!fileId) {
      logger?.err(
        `System::build_modules[${filename}]`,
    ...
    if (!processedFile) {
      logger?.err(
        `System::build_modules[${filename}]`,
        "Internal error, file never processed: " + filename
      );
      return false;
    }
    ...
    if ("errors" in processedFile) {
      logger?.fatal(
        `System::build_modules[${filename}]`,
        "Cannot continue compilation. Required shader did not pass validation: " +
          filename
      );
      return false;
    }
    ...
    if (entryName) {
      ...
      if (!entryPoint) {
        logger?.err(
          `System::build_modules[${filename}]`,
          "Entry point not found: " + entryName
        );
        return false;
      }
      if (entryPoint.stage !== "Fragment") {
        logger?.err(
          `System::build_modules[${filename}]`,
          "Quad pass entry point must be Fragment stage."
        );
        return false;
      }
    } else {
      ...
      if (!entryPoint) {
        logger?.err(
          `System::build_modules[${filename}]`,
          "Fragment entry point required for Quad pass pipeline."
        );
        return false;
      }
    }
    ...
    for (const globalVariable of global_variables) {
      if (!globalVariable.name) {
        logger?.err(
          `System::build_modules[${filename}]`,
          "Global variable does not have name."
        );
        continue;
      }
      ...
      if (!bindPath) {
        logger?.err(
          `System::build_modules[${filename}]`,
          "Bind not found in runner: " + globalVariable.name
        );
        continue;
      }
      ...
      if (!resource) {
        logger?.err(
          `Pipeline::build[${filename}]`,
          "Resource at path not found: " + bindPath
        );
        return false;
      }
      ...
      if (!!bindSlots[group][binding]) {
        logger?.err(
          `System::build_modules[${filename}]`,
          "Duplicate bindings for shader: " + filename
        );
        return false;
      }
    ...
    bindSlots.forEach((bind, group) =>
      bind.forEach((entry, binding) => {
        let re = new RegExp(
          "@group\\(999\\) @binding\\(999\\)(?=\\s+var\\s*(<[a-z]+>\\s+" +
            entry +
            "))",
          "g"
        );
        bindedShader = bindedShader?.replace(
          re,
    ...
    for (const [group_idx, group] of bindSlots.entries()) {
      ...
      if (!resources.every((r) => !!r)) {
        logger?.fatal(
          `System::build_modules[${filename}]`,
          "Unknown resource in binds"
        );
        return false;
      }
      let bindGroupLayoutEntries = resources.map((r, binding_idx) =>
        r!.getBindGroupLayoutEntry(
          binding_idx,
      ...
      let bindGroupEntries = resources.map((r, binding_index) =>
        r!.getBindGroupEntry(binding_index)
      );
      ...
      if (err1) {
        logger?.fatal(
          `System::build_modules[${filename}]`,
          "Error when contructing bind group layout: " + err1.message
        );
        return false;
      }
      ...
      if (err2) {
        logger?.fatal(
          `System::build_modules[${filename}]`,
          "Error when contructing bind group: " + err2.message
        );
        return false;
      }
    ...
    if (!compiledModule || moduleNeedCompile[fileId] === true) {
      logger?.trace(
        ...
        "Starting compilation for file " + filename
      );
    ...
    if (!compiledModule) {
      logger?.fatal(
        `Pipeline::build[${filename}]`,
        "Project failed compilation for above reasons"
      );
      return false;
    }
    ...
    for (const targetPath of targets) {
      ...
      if (!res) {
        logger?.fatal(
          `Pipeline::build[${filename}]`,
          "Invalid fragment target path: " + targetPath
        );
        return false;
      }
    ...
    if (err) {
      logger?.fatal(
        `System::build_modules[${filename}]`,
        "Error when contructing pipeline: " + err.message
      );
      return false;
    }
