...
export default class TextureResource implements types.ResourceInstance {
  ...
  constructor(label: string, texture: GPUTexture, width: number, height: number, format: GPUTextureFormat) {
    this.label = label
  ...
  public static build = async (
    ...
    if (textureCreationError) {
      logger?.err(`Resource::Texture[${returnLabel}]`, `Could not create texture due to the following error: ${textureCreationError}`)
      return undefined
    }
    ...
    let ret = new TextureResource(returnLabel, texture, width ?? bitmap!.width, height ?? bitmap!.height, textureDescriptor.format)
    return ret
  }
...
export class CanvasTextureResource extends TextureResource {
  ...
  public static fromId = async (
    ...
    if (!canvas) {
      logger?.err(`Resource::Texture[${label}]`, `Canvas does not exist: id=${canvasId}`)
      return undefined
    }

    const canvasContext = canvas.getContext('webgpu')!
    if (!canvasContext) {
      logger?.err(`Resource::Texture[${label}]`, `Failed to create webgpu context on canvas: id=${canvasId}`)
      return undefined
    }
    ...
    logger?.debug(`Resource::Texture[${label}]`, `Texture attached on canvas: id=${canvasId}`)
    return new CanvasTextureResource(canvasContext, canvasId, label, texture, presentationSize[0], presentationSize[1], preferredFormat)
  }

  constructor(canvasContext: GPUCanvasContext, canvasId: string, label: string, texture: GPUTexture, width: number, height: number, format: GPUTextureFormat) {
  ...
  destroy(logger?: Logger) {
    ...
    logger?.debug(`Resource::Texture[${this.label}]`, `Unconfiguring canvas context`)
